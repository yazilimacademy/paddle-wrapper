...
public class CustomerPortalSessionApiError : ApiError
{
    public CustomerPortalSessionApiError(string type, string code, string detail, string documentationUrl, List<FieldError> fieldErrors)
        : base(type, code, detail, documentationUrl, fieldErrors)
    {
    }

    public static CustomerPortalSessionApiError FromJson(JsonElement root)
    {
        JsonElement error = root.GetProperty("error");

        return new CustomerPortalSessionApiError(
