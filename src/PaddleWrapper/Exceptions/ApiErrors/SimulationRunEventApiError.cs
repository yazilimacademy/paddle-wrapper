...
public class SimulationRunEventApiError : ApiError
{
    public SimulationRunEventApiError(string type, string code, string detail, string documentationUrl, List<FieldError> fieldErrors)
        : base(type, code, detail, documentationUrl, fieldErrors)
    {
    }

    public static SimulationRunEventApiError FromJson(JsonElement root)
    {
        JsonElement error = root.GetProperty("error");

        return new SimulationRunEventApiError(
