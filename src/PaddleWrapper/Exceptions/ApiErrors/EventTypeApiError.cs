...
public class EventTypeApiError : ApiError
{
    public EventTypeApiError(string type, string code, string detail, string documentationUrl, List<FieldError> fieldErrors)
        : base(type, code, detail, documentationUrl, fieldErrors)
    {
    }

    public static EventTypeApiError FromJson(JsonElement root)
    {
        JsonElement error = root.GetProperty("error");

        return new EventTypeApiError(
