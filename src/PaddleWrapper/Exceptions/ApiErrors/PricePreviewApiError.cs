...
public class PricePreviewApiError : ApiError
{
    public PricePreviewApiError(string type, string code, string detail, string documentationUrl, List<FieldError> fieldErrors)
        : base(type, code, detail, documentationUrl, fieldErrors)
    {
    }

    public static PricePreviewApiError FromJson(JsonElement root)
    {
        JsonElement error = root.GetProperty("error");

        return new PricePreviewApiError(
