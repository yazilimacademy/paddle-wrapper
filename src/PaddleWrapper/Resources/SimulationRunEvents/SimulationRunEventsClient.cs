...
{
    public class SimulationRunEventsClient(Client client)
    {
        public async Task<SimulationRunEventCollection> ListAsync()
        {
            try
            {
                HttpResponseMessage response = await client.GetRawAsync("/simulation-run-events");
        ...
        public async Task<SimulationRunEvent> GetAsync(string id)
        {
            try
            {
                HttpResponseMessage response = await client.GetRawAsync($"/simulation-run-events/{id}");
                string jsonString = await response.Content.ReadAsStringAsync();
                JsonElement root = JsonDocument.Parse(jsonString).RootElement;
