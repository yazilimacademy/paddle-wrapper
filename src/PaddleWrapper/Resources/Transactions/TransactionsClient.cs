...
{
    public class TransactionsClient(Client client)
    {
        public async Task<TransactionCollection> ListAsync(ListTransactions listOperation = null)
        {
            try
            {
                HttpResponseMessage response = await client.GetRawAsync("/transactions", listOperation);
        ...
        public async Task<Transaction> GetAsync(string id, GetTransaction getOperation = null)
        {
            try
            {
                HttpResponseMessage response = await client.GetRawAsync($"/transactions/{id}", getOperation);
                string jsonString = await response.Content.ReadAsStringAsync();
                JsonElement root = JsonDocument.Parse(jsonString).RootElement;
