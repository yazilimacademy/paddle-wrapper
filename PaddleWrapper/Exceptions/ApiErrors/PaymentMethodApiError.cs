...
public class PaymentMethodApiError : ApiError
{
    public PaymentMethodApiError(string type, string code, string detail, string documentationUrl, List<FieldError> fieldErrors)
        : base(type, code, detail, documentationUrl, fieldErrors)
    {
    }

    public static PaymentMethodApiError FromJson(JsonElement root)
    {
        JsonElement error = root.GetProperty("error");

        return new PaymentMethodApiError(
